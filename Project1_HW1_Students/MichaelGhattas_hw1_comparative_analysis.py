...
# Student Name(s): Michael Ghattas
# ============================================================
...
if api_key:
    ...
    print(f"OpenRouter client created successfully! Key starts with: {api_key[:8]}")
else:
...
def load_and_examine_data(file_path):
    """Load the dataset and print basic inspection info."""
...
#   c) API call: client.chat.completions.create() with
#                model="gpt-4.0-mini", a system role, a user
...
def llm_data_cleaning(df, verbose=True):
    ...
    cleaning_prompt = f"""
You are a data cleaning assistant for health datasets.
Return ONLY the cleaned CSV with headers.
Do not include markdown, explanations, notes, or code fences.

EXAMPLE 1
Dirty Input:
patient_id,age,gender,weight_kg,height_cm,systolic_bp,diastolic_bp,cholesterol,smoking_status,diabetes,bmi,enrollment_date,date_of_birth
P001,45,male,70,175,120,80,200,never,Yes,22.9,2024-01-15,1979-01-15
P002,999,F,65,160,130,85,210,Former,0,25.4,2024-01-15,1980-01-15
P002,999,F,65,160,130,85,210,Former,0,25.4,2024-01-15,1980-01-15

Cleaned Output:
patient_id,age,gender,weight_kg,height_cm,systolic_bp,diastolic_bp,cholesterol,smoking_status,diabetes,bmi,enrollment_date,date_of_birth
P001,45,M,70,175,120,80,200,Never,1,22.9,2024-01-15,1979-01-15

EXAMPLE 2
Dirty Input:
patient_id,age,gender,weight_kg,height_cm,systolic_bp,diastolic_bp,cholesterol,smoking_status,diabetes,bmi,enrollment_date,date_of_birth
P010,,female,68,165,118,76,190,current,0,,2024-01-15,1990-01-15
P011,34,,NULL,170,missing,82,-50,Former,No,999.9,2024-01-15,
P012,33,F,72,168,125,80,195,former,0,25.5,2024-01-15,1991-01-15

Cleaned Output:
patient_id,age,gender,weight_kg,height_cm,systolic_bp,diastolic_bp,cholesterol,smoking_status,diabetes,bmi,enrollment_date,date_of_birth
P010,33.5,F,68,165,118,76,190,Current,0,25.5,2024-01-15,1990-01-15
P012,33,F,72,168,125,80,195,Former,0,25.5,2024-01-15,1991-01-15

Apply these rules:
1. Missing values: impute numeric with median or remove row if multiple values are missing. Leave missing categorical values as-is.
2. Outliers: remove rows with extreme or impossible values (age > 120, weight > 200 kg, BP > 250, negatives in positive-only columns).
3. Data types: cast to numeric where appropriate; standardize binary categoricals (Yes/No -> 1/0).
4. Formatting: standardize gender and smoking_status.
5. Duplicates: remove duplicate rows, keep first occurrence.
6. Date/age integrity: if date_of_birth and enrollment_date are present, compute age_from_dates and remove rows where absolute difference from age is greater than 2 years.
7. Structured missingness (MNAR): if missingness appears concentrated in a subgroup, do not impute with a global median. Leave values as-is or use subgroup-specific medians.

NOW CLEAN THIS DATASET:
{data_csv}
"""
    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You clean tabular health data and return ONLY valid CSV with headers. No explanations. No markdown."
                },
                {
                    "role": "user",
    ...
    if not valid_lines:
        print("Error: No valid CSV lines found in LLM response.")
        return None

    csv_text = "\n".join(valid_lines)

    try:
        df_cleaned = pd.read_csv(StringIO(csv_text))
    except Exception as e:
        print(f"Error parsing cleaned CSV: {e}")
        return None
